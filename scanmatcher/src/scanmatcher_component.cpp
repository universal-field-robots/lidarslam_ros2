...
namespace graphslam
{
        ...
        broadcaster_(this)
  {
    ...
    declare_parameter("rot_for_mapupdate", 0.3);
    get_parameter("rot_for_mapupdate", rot_for_mapupdate_);
    declare_parameter("use_max_transform", false);
    get_parameter("use_max_transform", use_max_transform_);
    declare_parameter("max_acceptable_trans", 0.2);
    get_parameter("max_acceptable_trans", max_acceptable_trans_);
    declare_parameter("max_acceptable_rot", 0.2);
    get_parameter("max_acceptable_rot", max_acceptable_rot_);
    ...
    declare_parameter("map_display_voxel_size", 0.5);
    get_parameter("map_display_voxel_size", display_map_voxel_size_);
    ...
    std::cout << "map_display_voxel_size:" << display_map_voxel_size_ << std::endl;
    std::cout << "------------------" << std::endl;
  ...
  void ScanMatcherComponent::initializePubSub()
  {
        ...
        [this](const typename geometry_msgs::msg::PoseStamped::SharedPtr msg) -> void
    {
      ...
      Eigen::Quaterniond q;
      q.x() = current_pose_stamped_.pose.orientation.x;
      q.y() = current_pose_stamped_.pose.orientation.y;
      q.z() = current_pose_stamped_.pose.orientation.z;
      q.w() = current_pose_stamped_.pose.orientation.w;
      previous_rot_mat_ = q.normalized().toRotationMatrix();
      initial_pose_received_ = true;
      ...
      const rclcpp::Time stamp)
  {
    ...
    // Ethan changes - enforce max acceptable transform
    bool exceeded_max_ = false;
    if (use_max_transform_) {
      Eigen::Vector3d trans_prev_ = sim_trans.block<3, 1>(0, 3).cast<double>();
      Eigen::Matrix3d rot_mat_prev_ = sim_trans.block<3, 3>(0, 0).cast<double>();
      Eigen::Vector3d trans_final_ = final_transformation.block<3, 1>(0, 3).cast<double>();
      Eigen::Matrix3d rot_mat_final_ = final_transformation.block<3, 3>(0, 0).cast<double>();

      double odom_trans_ = (trans_final_ - trans_prev_).norm();
      Eigen::Matrix3d rot_mat_ = rot_mat_final_ * rot_mat_prev_.transpose();
      double odom_rot_ = std::abs(std::acos(std::clamp((rot_mat_.trace() - 1) / 2, -1.0, 1.0)));

      if (odom_trans_ > max_acceptable_trans_ || odom_rot_ > max_acceptable_rot_) {
        RCLCPP_WARN(get_logger(), "Odometry transform exceeded accepted maximum - Translation: %f | Rotation: %f", odom_trans_, odom_rot_);
      ...
      const Eigen::Matrix4f final_transformation, const rclcpp::Time stamp)
  {
    ...
    Eigen::Matrix3d rot_mat_ = rot_mat * previous_rot_mat_.transpose();
    rot_ = std::abs(std::acos(std::clamp((rot_mat_.trace() - 1) / 2, -1.0, 1.0)));
    if (trans_ >= trans_for_mapupdate_ && rot_ >= rot_for_mapupdate_ && !mapping_flag_)
    {
      geometry_msgs::msg::PoseStamped corrent_pose_stamped;
      corrent_pose_stamped = current_pose_stamped_;
  ...
  void ScanMatcherComponent::publishMap()
  {
    ...
    voxel_filter.setLeafSize(display_map_voxel_size_, display_map_voxel_size_, display_map_voxel_size_);
    voxel_filter.filter(*voxel_map_ptr);

    sensor_msgs::msg::PointCloud2::SharedPtr map_msg_ptr(new sensor_msgs::msg::PointCloud2);
    pcl::toROSMsg(*voxel_map_ptr, *map_msg_ptr); // *map_ptr
